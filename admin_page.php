<?php
	...
	if(isset($_POST['upload_punjabi'])){
		...
		if(isset($_FILES['audio_file'])){
			...
			move_uploaded_file($file_tmp,"songs/punjabi_songs/".$file_name);
         }
         ...
         if(isset($_FILES['song_image'])){
			...
			move_uploaded_file($file_tmp,"songs/punjabi_songs/img/".$file_name);
		 }

		$sql = "INSERT INTO punjabi_albums(`song_name`,`song_format`,`singer_name`,`movie_name`,`song_image`,`audio_file`) VALUES('$song_name','$song_format','$singer_name','$movie_name','$song_image','$audio_file')";
		$result = mysqli_query($conn,$sql);
		if($result){
				echo '<script type="text/javascript">';
                echo 'setTimeout(function () { sweetAlert("Uploaded"," <b>Successfully uploaded '.$song_name.'</b>","success");';
